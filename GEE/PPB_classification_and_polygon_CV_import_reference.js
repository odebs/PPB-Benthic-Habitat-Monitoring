...
// =============================
var training = predictorImage.select(selectedBands).sampleRegions({
  collection: samples,
...
// ============================================================================
// 8. FUNCTION TO DISTRIBUTE WHOLE POLYGONS ACROSS THREE FOLDS
//
// IMPORTANT:
// Assignment occurs separately within each habitat class.
//
// Therefore:
// - every habitat class contributes polygons to all three folds;
// - pixels belonging to one polygon cannot occur in multiple folds;
// - no polygon is simultaneously used for training and validation.
//
// The smallest class contains three polygons, so 3-fold CV is appropriate.
// ============================================================================

var nFolds = 3;

function assignPolygonFolds(fc, seed) {

  // Randomise the order of polygons within this habitat class.
  var randomized = fc
    .randomColumn('cv_random', seed)
    .sort('cv_random');

  var n = randomized.size();

  var polygonList = randomized.toList(n);
  ...
  // Allocate polygons sequentially across folds:
  // 0, 1, 2, 0, 1, 2...
  var folded = indices.map(function(i) {

    i = ee.Number(i);

    var polygon =
      ee.Feature(polygonList.get(i));
  ...
  return ee.FeatureCollection(folded);
}


// ============================================================================
// 9. ASSIGN FOLDS SEPARATELY WITHIN EACH CLASS
// ============================================================================

var cv0 = assignPolygonFolds(
  Deep_water, 100
);

var cv1 = assignPolygonFolds(
  Sub_canopy_brown_and_Caulerpa_Biotope, 101
);

var cv2 = assignPolygonFolds(
  Ecklonia_Phyllospora_Communities, 102
);

var cv3 = assignPolygonFolds(
  Sublittoral_Seagrass_Beds, 103
);

var cv4 = assignPolygonFolds(
  Sublittoral_SeaweedCommunities_onSediment, 104
);

var cv5 = assignPolygonFolds(
  RockyReef, 105
);

var cv6 = assignPolygonFolds(
  Sediment, 106
);

var cv7 = assignPolygonFolds(
  Shoreline_veg, 107
);

var cv8 = assignPolygonFolds(
  Bare_rocks, 108
);
